...
namespace SparkleXRTemplates.MagicLeap.MagicLeapTools
{
    public class ManagedKeypoint
    {
        ...
        public void Update(InputDevice inputDevice, Vector3 keyPointLocation, params Vector3[] decayPoints)
        {

            if (inputDevice.TryGetFeatureValue(MagicLeapHandUsages.Confidence, out float deviceHandConfidence) && (deviceHandConfidence < 0.85f))
            { 
                //lost:
                if (Visible)
                {
